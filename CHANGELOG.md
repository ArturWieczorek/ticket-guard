...
- **Firestore free-tier read usage.** The event view polled the database every 5s
  ...
  daily read quota (making the app show "can't verify"). The event view now has
  **no background polling** - it updates only when you click **Refresh** - so
  leaving it open costs zero reads; scan counts are tracked locally between the
  single scan-start read.
